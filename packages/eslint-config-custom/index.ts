...
// CommonJS on purpose: consumers include plain `.cjs` flat configs (loaded with
// require()) as well as `.ts` configs loaded through ESLint's jiti loader, and a
// CJS module body works from both (jiti's interopDefault returns module.exports).
...
const eslintRecommendedRules: Record<string, 'error'> = {};
...
module.exports = [
  {
    ...
    // eslint config file per linted file, so shared ignores must live here
